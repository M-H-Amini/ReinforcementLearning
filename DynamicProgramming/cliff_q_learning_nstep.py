...
class MHAgent:
    def __init__(self, gamma=1, eps=0.5, alpha=0.5, n=1):
        self.hist_s = []
        self.hist_a = []
        self.hist_r = []
        self.hist_s_prime = []
    ...
    def updateBuffer(self, s, a, s_prime, r, done, alpha=None):
        ...
        t = len(self.hist_s) - self.n
        if t >= 0:
            self.v, self.q = self.updateQ(t, self.n)
        if done:
            for i in range(1, self.n):
                if len(self.hist_s) - i >= 0:
                    self.v, self.q = self.updateQ(len(self.hist_s)-i, i)
            self.clearBuffer()
        self.improvePolicy()
        return self.v, self.q
    
    def getReward(self, t):
        return self.hist_r[t] if t in range(len(self.hist_r)) else 0

    def findG(self, t, n=1):
        g = sum([self.getReward(i) * self.gamma ** (i-t) for i in range(t, t+n)])
        if t+n <= len(self.hist_s):
            g += (self.gamma ** n) * max([self.q[self.hist_s_prime[t+n-1], a] for a in range(4)])
        return g
    ...
    def updateQ(self, t, n, alpha=None):
        alpha = self.alpha if alpha is None else alpha
        s, a = self.hist_s[t], self.hist_a[t]
        g = self.findG(t, n)
...
mha = MHAgent(gamma=0.99, eps=0.1, alpha=0.8, n=2) 
