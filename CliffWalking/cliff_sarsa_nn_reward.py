...
class MHAgent:
    ...
    def updateBuffer(self, s, a, s_prime, r, done, alpha=None):
        ...
        if done:
            self.n_episodes -=- 1 
            if self.n_episodes and not(self.n_episodes % 50):
...
def getReward(reward, s_prime):
    if reward == -100:
        return -100
    r, c = state2Coord(s_prime)
    reward = 0 
    reward += r 
    reward += c
    return reward
...
mha = MHAgent(gamma=0.99, eps=0.4, alpha=0.001, landa=0.9) 
...
while True:
    ...
    reward = getReward(reward, observation)
    G += reward
