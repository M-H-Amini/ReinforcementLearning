...
def visualize(v):
    ...
    plt.xlabel('Column')
    plt.ylabel('Row')
    plt.savefig('value_expected_sarsa.png')
    plt.show()
    plt.close('all')
...
def visualizePolicy(q):
    ...
    plt.xlabel('Column')
    plt.ylabel('Row')
    plt.savefig('policy_expected_sarsa.png')
    plt.show()
    plt.close('all')
...
class MHAgent:
    ...
    def updateQ(self, alpha=None):
        if len(self.hist_s):
            ...
            expected = sum([self.q[s_prime, a]*self.probs[s_prime, a] for a in range(4)])
            self.q[s, a] = self.q[s, a] + alpha * (r + self.gamma * expected - self.q[s, a])
...
mha = MHAgent(gamma=0.99, eps=0.2, alpha=0.5) 
