...
def prettyPrintQ(q):
    for i in range(len(q)):
        if not (q[i] == np.zeros_like(q[i])).all():
            print(f'q[{i}, ...] = {q[i]}')

def prettyPrintP(q):
    for i in range(len(q)):
        if not (q[i] == np.ones_like(q[i]) * 0.25).all():
            print(f'p[{i}, ...] = {q[i]}')
...
class MHAgent:
    ...
    def updateBuffer(self, s, a, s_prime, r, done, alpha=None):
        ...
        logger.debug(f'Buffer updated: s: {s}, a: {a}, s_prime: {s_prime}, r: {r}, done: {done}')
        logger.debug(f'hist_s: {self.hist_s}')
        logger.debug(f'hist_a: {self.hist_a}')
        logger.debug(f'hist_s_prime: {self.hist_s_prime}')
        logger.debug(f'hist_r: {self.hist_r}')
        logger.debug(f'hist_done: {self.hist_done}')
        
        if len(self.hist_s) > 1:
            s, a, r, s_prime = self.hist_s[-2], self.hist_a[-2], self.hist_r[-2], self.hist_s_prime[-2]
            a_prime = self.hist_a[-1]
            log.debug(f'Updating Q for s: {s}, a: {a}, r: {r}, s_prime: {s_prime}, a_prime: {a_prime}')
        ...
        if done:
            ...
            logger.debug(f'Updating Q for done... s: {s}, a: {a}, r: {r}, s_prime: {s_prime}, a_prime: {a_prime}')
            self.v, self.q = self.updateQ(s, a, r, s_prime, a_prime)
            self.clearBuffer()
            # logger.debug(f'q[{s}, ...] = {self.q[s]}')
            self.n_episodes -=- 1
    ...
    def updateQ(self, s, a, r, s_prime, a_prime, alpha=None):
        logger.debug('Inside updateQ...')
        logger.debug(f's: {s}, a: {a}, r: {r}, s_prime: {s_prime}, a_prime: {a_prime}')
        alpha = self.alpha if alpha is None else alpha
        g = r + self.gamma * self.q[s_prime, a_prime]
        logger.debug(f'g: {g}')
        self.q[s, a] = self.q[s, a] + alpha * (g - self.q[s, a])
        logger.debug(f'q[{s}, {a}]: {self.q[s, a]}')
...
mha = MHAgent(gamma=0.99, eps=0.3, alpha=0.1) 
