...
def saveStats(name, episode_hist):
    np.save(name, np.array(episode_hist))
...
for experiment in range(experiment_no):
    model = buildModel()
    model_train = buildModelTrain(model)
    ...
    episode_hist.append(episode_lens)
    saveStats('nstep.npy', episode_hist)
